...
function renderPlanningUI(root, plan, {
  ...
  const collapsedSections = new Set(
    [...root.querySelectorAll("details.collapsible-section:not([open])")].map((details) => details.id),
  );
  ...
  root.innerHTML = `
    ${section("athlete", "1", "Athlete and Equipment", `
      <div class="field-grid athlete-layout">
      ${showUnits ? `<div class="athlete-units-row">${selectField("Units", "units", [["imperial", "Imperial"], ["metric", "Metric"]], plan.units, "", "Changes the displayed weight and the units used for route and fueling measurements.")}</div>` : ""}
      <div class="athlete-row athlete-weight-row">
        ${numberField(`Rider Weight (${imperial ? "lb" : "kg"})`, "athlete.riderWeightKg", weight(plan.athlete.riderWeightKg), disabled, "0.1", "Rider body mass. Combined with bike and carried equipment for climbing and rolling-resistance estimates.", imperial ? 350 : 160)}
        ${numberField(`Bike Weight (${imperial ? "lb" : "kg"})`, "athlete.bikeWeightKg", weight(plan.athlete.bikeWeightKg), disabled, "0.1", "Bike mass used with rider and carried equipment in Meet Target Duration calculations.", imperial ? 80 : 36)}
        ${numberField(`Carried Equipment (${imperial ? "lb" : "kg"})`, "athlete.carriedWeightKg", weight(plan.athlete.carriedWeightKg), disabled, "0.1", "Clothing, bottles, tools, and other carried mass included in total system weight.", imperial ? 60 : 27)}
      </div>
      <div class="athlete-row athlete-power-row">
        ${numberField("Threshold Power", "athlete.thresholdPower", plan.athlete.thresholdPower, disabled, "1", "Sustainable threshold power used to calculate relative intensity, workload, and profile difficulty.", 600)}
        ${numberField("Low Threshold Power", "athlete.lowThresholdPower", plan.athlete.lowThresholdPower, disabled, "1", "The transition from steady aerobic riding toward harder sustained effort. Retained as part of the athlete model.", 500)}
        ${numberField("Peak Power", "athlete.peakPower", plan.athlete.peakPower, disabled, "1", "Maximum short-duration power retained for sprint-oriented planning and shared Power Master parameters.", 2000)}
      </div></div>`, sectionActions)}
    ${section("route", "2", "Route and Duration", `
      ...
      <div class="field-grid field-grid--six">
        ...
        ${numberField(`Activity Distance (${imperial ? "mi" : "km"})`, "route.distanceKm", distance, locked || routeDerived ? "disabled" : "", "0.1", "Distance from the loaded activity route, or a manual route distance when no activity is loaded.", imperial ? 500 : 800)}
        ${numberField(`Elevation Gain (${imperial ? "ft" : "m"})`, "route.elevationGainM", elevation, locked || routeDerived ? "disabled" : "", "1", "Total ascent used by the route-physics estimate and route-oriented profile matching.", imperial ? 30000 : 9000)}
        ${numberField("Moving Duration (min)", "route.movingDurationMinutes", number(plan.route.movingDurationMinutes), disabled, "0.1", "Planned time spent moving. Power workload and planned average speed use this duration.", 1440)}
        ${numberField("Stopped Duration (min)", "route.stoppedDurationMinutes", number(plan.route.stoppedDurationMinutes), disabled, "0.1", "Planned stops, aid stations, traffic, and rest. Fueling uses moving plus stopped duration.", 360)}
        ${numberField("Climbing Duration (min)", "route.climbingDurationMinutes", number(plan.route.climbingDurationMinutes), disabled, "0.1", "Minutes spent on uphill segments at a grade of 2% or steeper. Parsed from supported activity files when possible.", 720)}
        ${numberField("Decline Duration (min)", "route.declineDurationMinutes", number(plan.route.declineDurationMinutes), disabled, "0.1", "Minutes spent on downhill segments at a grade of -2% or steeper. Parsed from supported activity files when possible.", 720)}
      </div>`, sectionActions)}
    ${section("resistance", "3", "Ride Resistance", `
      <div class="field-grid field-grid--three">
        ...
        ${numberField("CdA (m²)", "resistance.cda", plan.resistance.cda, disabled, "0.01", "Aerodynamic drag area. Lower values require less power at higher speed.", 0.6, 0.1)}
        ${numberField("Rolling Resistance (Crr)", "resistance.crr", plan.resistance.crr, disabled, "0.001", "Rolling-resistance coefficient for the tires and road surface. Typical road values are around 0.003 to 0.008.", 0.02, 0.001)}
      </div>`, sectionActions)}
    ${section("demand", "4", "Demand and Profile", `
      <div class="field-grid field-grid--six">
        ...
        ${numberField("Sprint Demand", "demand.sprint", number(plan.demand.sprint), demandDisabled, "0.1", "Workload from very short explosive efforts near maximum power, such as sprints or sharp accelerations.", 75)}
      </div>`, sectionActions)}
  `;
  ...
  root.onclick = (event) => {
    ...
    if (action) {
      ...
      if (!action.disabled) sectionActions[action.dataset.sectionAction]?.(action.dataset.section);
    }
...
function section(id, step, title, body, actions) {
  ...
  return `<details id="${anchor}" class="card collapsible-section" open><summary class="section-title"><div class="section-heading"><span>${step}</span><h2>${title}</h2></div><div class="section-actions">
    ${sectionLink(anchor, title)}
...
function linkIcon() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.5.5l2-2a5 5 0 0 0-7-7l-1.2 1.2"/><path d="M14 11a5 5 0 0 0-7.5-.5l-2 2a5 5 0 0 0 7 7l1.2-1.2"/></svg>`; }
function pinIcon() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 3h6l-1 6 3 3v2H7v-2l3-3-1-6Z"/><path d="M12 14v7"/></svg>`; }
function sectionLink(anchor, title) { return `<button type="button" class="icon-button section-link" data-section-link="${anchor}" title="Copy link to ${title}" aria-label="Copy link to ${title}">${linkIcon()}</button>`; }
function stickyButton(anchor) { return `<button type="button" class="icon-button section-sticky-toggle" data-sticky-target="${anchor}" title="Pin while scrolling" aria-label="Pin while scrolling" aria-pressed="false">${pinIcon()}</button>`; }

function numberField(label, path, value, disabled, step = "0.1", help = "", sliderMax = null, sliderMin = 0) {
  const demandClass = path === "demand.aerobic" ? " demand-low" : path === "demand.hardEffort" ? " demand-high" : path === "demand.sprint" ? " demand-peak" : "";
  const powerZoneClass = path === "athlete.lowThresholdPower" ? "power-zone power-zone--low"
    : path === "athlete.thresholdPower" ? "power-zone power-zone--threshold"
      : path === "athlete.peakPower" ? "power-zone power-zone--peak" : "";
  const slider = sliderMax == null ? "" : `data-slider-min="${sliderMin}" data-slider-max="${sliderMax}" data-slider-step="${step}"`;
  return `<label class="field ${powerZoneClass}"><span class="field-label">${label}${tooltip(help)}</span><input class="${demandClass.trim()}" type="number" step="${step}" min="${sliderMin}" value="${value}" data-path="${path}" ${slider} ${disabled}></label>`;
}
